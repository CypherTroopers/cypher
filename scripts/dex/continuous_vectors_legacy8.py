#!/usr/bin/env python3
"""Independent G3 fixture arithmetic, using the pre-existing Python model.
...
def calculate():
    ...
    fees = {period: 0 for period in range(1, 9)}
    closes = {18: 1, 34: 2, 38: 3, 54: 4, 58: 5, 74: 6, 78: 7, 88: 8}
    mark = 100 * ATOMS
    trace, periods = [], []
    ...
    def check(height, event):
        ...
        trace.append({"height": height, "event": event, "custody": custody,
                      "accounts": {key: account["cash"] for key, account in accounts.items()},
                      "buckets": dict(buckets)})

    check(0, "authenticated_initial_funding_assumed")
    for height in range(1, 91):
        ...
        if height <= 80 and offset in (7, 11):
            mark = (100 if offset == 7 else 110) * ATOMS
            check(height, "oracle")
        if height <= 80 and offset in (9, 13):
            opening = offset == 9
    ...
    expected = {"U": "0", "T": "279.664", "F": "0.168", "S": "12.168",
                "I": "5", "Z": "0", "W": "0", "R": "0"}
    ...
    assert inputs == atoms("345") and custody == atoms("297")
    assert withdrawal_paid == atoms("40") and reward_paid == atoms("8")
    ...
    return {"version": 1, "scope": "independent arithmetic only; authentication and transfers assumed",
...
def main():
    ...
    result = calculate()
    if args.check:
        print("PASS: four cycles, eight periods, inputs345 - withdrawals40 - rewards8 = custody297; "
              "T279.664 F0.168 S12.168 I5 Z0; Alice159.796 Bob119.868 CLX")
    else:

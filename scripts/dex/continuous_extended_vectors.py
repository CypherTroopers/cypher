#!/usr/bin/env python3
"""Independent G3 extended10 fixture arithmetic; legacy8 is preserved separately.
...
def calculate():
    ...
    return {"version": 1, "fixture": "extended10",
...
def main():
    ...
    result = calculate()
    if args.check:
        golden = Path(__file__).resolve().parents[2] / "dex/testdata/continuous-extended10.json"
        assert json.loads(golden.read_text()) == result, "extended10 golden differs"
        print("PASS: extended10, four trade cycles plus flat catch-up interval, "

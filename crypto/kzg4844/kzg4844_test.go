...
func TestCalcBlobHashV1Prefix(t *testing.T) {
	blob := randBlob()
	commitment, err := BlobToCommitment(blob)
	if err != nil {
		t.Fatalf("failed to create KZG commitment from blob: %v", err)
	}
	hash := CalcBlobHashV1(sha256.New(), &commitment)
	if hash[0] != 0x01 {
		t.Fatalf("unexpected versioned hash prefix: got %x want 0x01", hash[0])
	}
	if !IsValidVersionedHash(hash[:]) {
		t.Fatalf("expected versioned hash to be valid")
	}
}
...
// goos: darwin
// goarch: arm64
// pkg: github.com/cypherium/cypher/crypto/kzg4844
// cpu: Apple M1 Pro

...
func opSuicide(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	beneficiaryValue := callContext.stack.pop()
	beneficiary := common.Address(beneficiaryValue.Bytes20())
	contractAddr := callContext.contract.Address()
	balance := interpreter.evm.StateDB.GetBalance(contractAddr)
	if interpreter.evm.chainRules.IsCancun && !interpreter.evm.StateDB.CreatedContract(contractAddr) {

...
func TestCypheriumCustomGenesisConfigPinsUpgradeSurface(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to read genesistest.json: %v", err)
	}

	var genesis Genesis
	if err := json.Unmarshal(blob, &genesis); err != nil {
		t.Fatalf("failed to decode genesistest.json: %v", err)
	}
	if genesis.Config == nil {
		t.Fatal("genesistest.json has no chain config")
	}
	if genesis.Config.ChainID == nil || genesis.Config.ChainID.Sign() == 0 {
		t.Fatal("genesistest.json must keep a non-zero chainId")
	}
...
func TestCypheriumModernForkConfigDecoding(t *testing.T) {
	...
	if !cfg.IsBerlin(big.NewInt(0)) {
		t.Fatal("Berlin must be active at genesis for genesistest.json")
	}
	if !cfg.IsLondon(big.NewInt(0)) {
		t.Fatal("London must be active at genesis for genesistest.json")
	}
	if !cfg.IsShanghai(big.NewInt(0), 0) {
		t.Fatal("Shanghai must be active at genesis timestamp for genesistest.json")
	}
	if !cfg.IsCancun(big.NewInt(0), 0) {
		t.Fatal("Cancun must be active at genesis timestamp for genesistest.json")
	}
	if cfg.IsPrague(big.NewInt(0), 0) {
		t.Fatal("Prague must remain disabled for genesistest.json")
	}
	if cfg.IsOsaka(big.NewInt(0), 0) {
		t.Fatal("Osaka must remain disabled for genesistest.json")
	}

...
def _write_failed_files(failed: Iterable[Path]) -> None:
    ...
    if not failed_list or not FAILED_FILES_LOG:
        return
    log.warning("Logging %d failed files to %s", len(failed_list), FAILED_FILES_LOG)
...
def build_index(persist_dir: str | None = None):
    ...
    if not paths:
        raise RuntimeError(
            "No source files found (paths is empty). Check SRC_DIR and file_filters.py"
        )
    ...
    if not docs:
                raise RuntimeError(
            "No documents loaded. Some readers may have failed or all files were ignored."
        )
    ...
    if not nodes:
        raise RuntimeError(
            "No nodes were generated from documents. Check CHUNK_SIZE/OVERLAP or input docs."
        )

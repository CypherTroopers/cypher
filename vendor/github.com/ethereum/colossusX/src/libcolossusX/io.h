...
// Reference: https://github.com/ethereum/wiki/wiki/colossusX-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see colossusX_io_prepare
enum colossusX_io_rc {
...
/**
 * Logs a critical error in important parts of colossusX. Should mostly help
...
/**
 * Prepares io for colossusX
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the colossusX
 ...
 *                           https://github.com/ethereum/wiki/wiki/colossusX-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum colossusX_io_rc
 */
enum colossusX_io_rc colossusX_io_prepare(
	char const* dirname,
	colossusX_h256_t const seedhash,
	FILE** output_file,
	uint64_t file_size,
	bool force_create
);
 ...
 */
FILE* colossusX_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* colossusX_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool colossusX_mkdir(char const* dirname);
 ...
 */
bool colossusX_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* colossusX_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/ethereum/wiki/wiki/colossusX-DAG
 *
 ...
 */
bool colossusX_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool colossusX_io_mutable_name(
	uint32_t revision,

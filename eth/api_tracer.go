		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					vmctx := core.NewEVMContextWithConfig(api.eth.blockchain.Config(), msg, task.block.Header(), api.eth.blockchain, nil)
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	return api.blockscoutTraceBlock(ctx, block, config)
}
...
func (api *PrivateDebugAPI) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range block.Transactions() {
		...
		var (
			msg, _ = tx.AsMessage(signer)
			vmctx  = core.NewEVMContextWithConfig(api.eth.blockchain.Config(), msg, block.Header(), api.eth.blockchain, nil)
		...
		vmenv, nativeCheck := core.NewEVMForNativeSimulation(vmctx, statedb, api.eth.blockchain.Config(), vmConf, msg.To())
		_, err = core.ApplyMessage(vmenv, msg, new(core.GasPool).AddGas(msg.Gas()))
		if nativeErr := nativeCheck(); nativeErr != nil {
			err = nativeErr
		}
		if writer != nil {
			writer.Flush()
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	for block.NumberU64() < origin {
		...
		nextNumber := block.NumberU64() + 1
		if block = api.eth.blockchain.GetBlockByNumber(nextNumber); block == nil {
			return nil, fmt.Errorf("block #%d not found", nextNumber)
		}
		_, _, _, err := api.eth.blockchain.Processor().Process(block, statedb, vm.Config{})
		if err != nil {
		...
		if root != block.Root() {
			return nil, fmt.Errorf("reexecuted state root mismatch at block %d", block.NumberU64())
		}
		if err := statedb.Reset(root); err != nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	return api.blockscoutTraceTransaction(ctx, hash, config)
}
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv, nativeCheck := core.NewEVMForNativeSimulation(vmctx, statedb, api.eth.blockchain.Config(), vm.Config{Debug: true, Tracer: tracer}, message.To())
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewEVMContextWithConfig(api.eth.blockchain.Config(), msg, block.Header(), api.eth.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv, nativeCheck := core.NewEVMForNativeSimulation(context, statedb, api.eth.blockchain.Config(), vm.Config{}, msg.To())

...
func main() {
	...
	scanConfig := overrideIstanbul(chainConfig, cfg.istanbulBlock)

	engine, err := chooseEngine(cfg.engine, scanConfig, db)
	if err != nil {
	...
	ctx := &chainContext{db: db, config: scanConfig, engine: engine}
	stateDB := state.NewDatabase(db)

	fmt.Printf("Scanning blocks %d..%d with IstanbulBlock=%s\n", cfg.start, endNumber, scanConfig.IstanbulBlock.String())
	for number := cfg.start; number <= endNumber; number++ {
		...
		receipts, root, receiptRoot, err := applyBlock(scanConfig, ctx, block, statedb)
		if err != nil {
...
func overrideIstanbul(config *params.ChainConfig, block int64) *params.ChainConfig {
	copy := *config
	if block < 0 {
		copy.IstanbulBlock = new(big.Int).SetUint64(math.MaxUint64)
		return &copy
	}
	copy.IstanbulBlock = big.NewInt(block)
	return &copy
}
...
func applyBlock(config *params.ChainConfig, ctx *chainContext, block *types.Block, statedb *state.StateDB) (types.Receipts, common.Hash, common.Hash, error) {
...
func (e *noRewardEngine) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, totalGas uint64) {
	header.Root = state.IntermediateRoot(chain.Config().IsEIP158(header.Number))
}

func (e *noRewardEngine) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	header.Root = state.IntermediateRoot(chain.Config().IsEIP158(header.Number))

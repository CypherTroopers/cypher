...
func TestFairHotstuffConfigAllowsCommonRPCWithoutRegistration(t *testing.T) {
	config := testFairHotstuffConfig()
	if err := config.CheckConfigForkOrder(); err != nil {
		t.Fatalf("Fair HotStuff requires Common RPC registration: %v", err)
	}
	...
	if _, exists := fields["commonRPCSigners"]; exists {
		t.Fatal("genesis configuration contains a Common RPC registration list")
	}
	var decoded ChainConfig
	if err := json.Unmarshal(encoded, &decoded); err != nil {
		t.Fatal(err)
	}
	if err := decoded.CheckConfigForkOrder(); err != nil {
		t.Fatalf("Common RPC participation changed after config round trip: %v", err)
	}
}

...
$STATIC_NODES_FILE = Join-Path $PEER_DIR "static-nodes.toml"
$TRUSTED_NODES_FILE = Join-Path $PEER_DIR "trusted-nodes.toml"
...
if (!(Test-Path -Path $STATIC_NODES_FILE -PathType Leaf)) {
  Write-Host "==> static-nodes.toml not found"
...
'@
  ...
  [System.IO.File]::WriteAllText($STATIC_NODES_FILE, $StaticNodesToml, $Utf8NoBom)

} else {
  Write-Host "==> Existing static-nodes.toml detected: $STATIC_NODES_FILE"
  Write-Host "==> Skip static-nodes.toml generation"
}

if (!(Test-Path -Path $TRUSTED_NODES_FILE -PathType Leaf)) {
  Write-Host "==> trusted-nodes.toml not found"
  Write-Host "==> Copy static-nodes.toml to trusted-nodes.toml"

  Copy-Item -Path $STATIC_NODES_FILE -Destination $TRUSTED_NODES_FILE -Force
} else {
  Write-Host "==> Existing trusted-nodes.toml detected: $TRUSTED_NODES_FILE"
  Write-Host "==> Skip trusted-nodes.toml generation"
}

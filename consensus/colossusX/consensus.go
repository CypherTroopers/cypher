...
func (colossusX *colossusX) CalcKeyBlockDifficulty(chain types.KeyChainReader, time uint64, parent *types.KeyBlockHeader) *big.Int {
...
func (colossusX *colossusX) VerifyCandidate(chain types.KeyChainReader, candidate *types.Candidate) error {
	...
	if colossusX.config.PowMode == ModeFake || colossusX.config.PowMode == ModeFullFake {
		// time.Sleep(colossusX.fakeDelay)
		if colossusX.fakeFail == candidate.KeyCandidate.Number.Uint64() {
	...
	if colossusX.shared != nil {
		return colossusX.shared.VerifyCandidate(chain, candidate)
	}
	...
	digest, result := colossusXlight(size, cache.cache, candidate.HashNoNonce().Bytes(), candidate.KeyCandidate.Nonce.Uint64())
	// Caches are unmapped in a finalizer. Ensure that the cache stays live
	// until after the call to colossusXlight so it's not unmapped while being used.
...
// candidate to conform to the colossusX protocol. The changes are done inline.
func (colossusX *colossusX) PrepareCandidate(chain types.KeyChainReader, candidate *types.Candidate, committeeSize int) error {
...
func (colossusX *colossusX) PowMode() uint {
...
func (colossusX *colossusX) Author0(header *types.Header) (common.PublicKey25519, error) {
...
func (colossusX *colossusX) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
	...
	return colossusX.verifyHeader(chain, header, parent, false, seal)
}
	...
	for i := 0; i < workers; i++ {
		go func() {
			for index := range inputs {
				errors[index] = colossusX.verifyHeaderWorker(chain, headers, seals, index)
				done <- index
			}
		}()
	}
...
func (colossusX *colossusX) verifyHeaderWorker(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool, index int) error {
	...
	return colossusX.verifyHeader(chain, headers[index], parent, false, seals[index])
}

// verifyHeader checks whether a header conforms to the consensus rules of the
// stock Ethereum colossusX engine.
// See YP section 4.3.4. "Block Header Validity"
func (colossusX *colossusX) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header, uncle bool, seal bool) error {
	...
	expected := colossusX.CalcDifficulty(chain, header.Time, parent)
	...
	*/
	...
	if seal {
		if err := colossusX.VerifySeal(chain, header); err != nil {
			return err
		}
	}

...
// datasetSize returns the size of the colossusX mining dataset that belongs to a certain
// block number.
...
func blake3_512(dest []byte, data []byte) {
...
func buildBlake3MerkleRoot(leaves [][]byte) []byte {
        if len(leaves) == 0 {
                return make([]byte, 32)
        }
        ...
        for len(level) > 1 {
                ...
                for i := 0; i < len(level); i += 2 {
                        ...
                        next = append(next, blake3_256(combined))
                }
                level = next
        }
        return level[0]
...
func shouldSampleAuditCell(round int) bool {
        if auditCellCount <= 0 {
                return false
        }
        stride := loopAccesses / auditCellCount
        if stride <= 0 {
                stride = 1
        }
...
func generateCache(dest []uint32, epoch uint64, seed []byte) {
        ...
        blake3_512(cache, seed)
        for offset := uint64(hashBytes); offset < size; offset += hashBytes {
                blake3_512(cache[offset:], cache[offset-hashBytes:offset])
        ...
        for i := 0; i < cacheRounds; i++ {
                for j := 0; j < rows; j++ {
                        ...
                        bitutil.XORBytes(temp, cache[dstOff:dstOff+hashBytes], cache[xorOff:xorOff+hashBytes])
                        blake3_512(cache[dstOff:], temp)
...
// prepare converts an colossusX cache or dataset from a byte stream into the internal
// int representation. All colossusX methods work with ints to avoid constant byte to
...
// fnvHash mixes in data into mix using the colossusX fnv method.
...
// generateDatasetItem combines data from pseudorandomly selected cache nodes
// into a 256-byte DAG cell.
func generateDatasetItem(cache []uint32, index uint32, sha3_512 hasher) []byte {
        ...
        // Expand the 64-byte seed into a 256-byte DAG cell.
        cell := make([]byte, mixBytes)
        copy(cell[:hashBytes], mix)

        stream := make([]byte, hashBytes+4)
        copy(stream[:hashBytes], mix)
        for lane := uint32(1); lane < mixBytes/hashBytes; lane++ {
                binary.LittleEndian.PutUint32(stream[hashBytes:], index^lane)
                sha3_512(cell[lane*hashBytes:(lane+1)*hashBytes], stream)
        }
        return cell
}

// generateDataset generates the entire colossusX dataset for mining.
// This method places the result into dest in machine byte order.
func generateDataset(dest []uint32, epoch uint64, cache []uint32) {
        ...
        for i := 0; i < threads; i++ {
                go func(id int) {
                        ...
                        cellHasher := hasher(blake3_512)

                        // Calculate the data segment this thread should generate
                        cells := size / mixBytes
                        batch := uint32((cells + uint64(threads) - 1) / uint64(threads))
                        first := uint32(id) * batch
                        limit := first + batch
                        if limit > uint32(cells) {
                                limit = uint32(cells)
                        }
                        // Calculate the dataset segment
                        percent := uint32(cells / 100)
                        if percent == 0 {
                                percent = 1
                        }
                        for index := first; index < limit; index++ {
                                item := generateDatasetItem(cache, index, cellHasher)
                                if swapped {
                                        swap(item)
                                }
...
// colossusXhash aggregates data from the full dataset in order to produce the final
...
func colossusXhash(hash []byte, nonce uint64, size uint64, lookup func(index uint32) []uint32) ([]byte, []byte) {
        ...
        initialHash := make([]byte, hashBytes)
        blake3_512(initialHash, seed)
        ...
        for i := 0; i < loopAccesses; i++ {
                ...
                if shouldSampleAuditCell(i) && len(accessLeaves) < auditCellCount {
                        accessLeaves = append(accessLeaves, blake3_256(cellBytes))
                }

                for lane := 0; lane < mixBytes/hashBytes; lane++ {
                        base := lane * hashWords
                        acc := [4]uint32{
                                mix[(lane*4)%hashWords],
                                mix[(lane*4+1)%hashWords],
                                mix[(lane*4+2)%hashWords],
                                mix[(lane*4+3)%hashWords],
                        }
                        for j := 0; j < hashWords; j++ {
                                acc[j%4] = fnv(acc[j%4], cell[base+j])
                        }
                        mix[(lane*4)%hashWords] = acc[0]
                        mix[(lane*4+1)%hashWords] = acc[1]
                        mix[(lane*4+2)%hashWords] = acc[2]
                        mix[(lane*4+3)%hashWords] = acc[3]
                }
        }

        digest := make([]byte, common.HashLength)
        mixDigestState := make([]byte, hashBytes)
        for i := range mix {
                binary.LittleEndian.PutUint32(mixDigestState[i*4:], mix[i])
        }
        // Single SHA3-512 compression after all rounds to avoid a per-round long dependency chain.
        sha3_512 := makeHasher(sha3.New512())
        sha3_512(mixDigestState, mixDigestState)
        merkleRoot := buildBlake3MerkleRoot(accessLeaves)
        digestInput := make([]byte, 0, len(mixDigestState)+len(merkleRoot))
        digestInput = append(digestInput, mixDigestState...)
        digestInput = append(digestInput, merkleRoot...)
        mixDigest := blake3.Sum256(digestInput)
        copy(digest, mixDigest[:])

        finalInput := make([]byte, 0, len(initialHash)+len(digest)+len(merkleRoot))
        finalInput = append(finalInput, initialHash...)
        finalInput = append(finalInput, digest...)
        finalInput = append(finalInput, merkleRoot...)
        result := blake3.Sum256(finalInput)
        return digest, result[:]
}

// colossusXlight aggregates data from the full dataset (using only a small
// in-memory cache) in order to produce our final value for a particular header
// hash and nonce.
func colossusXlight(size uint64, cache []uint32, hash []byte, nonce uint64) ([]byte, []byte) {
        cellHasher := hasher(blake3_512)

        lookup := func(index uint32) []uint32 {
                rawData := generateDatasetItem(cache, index, cellHasher)
        ...
        return colossusXhash(hash, nonce, size, lookup)
}

// colossusXfull aggregates data from the full dataset (using the full in-memory
...
// datasetSizes is a lookup table for the colossusX dataset size for the first 2048
...
// cacheSizes is a lookup table for the colossusX verification cache size for the
